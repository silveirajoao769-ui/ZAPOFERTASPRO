...
export async function POST(req:NextRequest){
 try{
  ...
  if(!(sub?.subscription_status==='active'||(sub?.subscription_status==='trialing'&&!!sub.trial_ends_at&&new Date(sub.trial_ends_at).getTime()>Date.now())))return NextResponse.json({error:'É necessário um plano ativo ou teste válido.'},{status:403})
  ...
  const limit=sub?.plan==='premium'?100:sub?.plan==='pro'?40:10
  const {data:permitted,error:quotaError}=await supabase.rpc('consume_ai_miner_quota',{p_user:user.id,p_limit:limit})
  if(quotaError)return NextResponse.json({error:'Não foi possível verificar seu limite mensal.'},{status:503})
  if(!permitted)return NextResponse.json({error:'Seu limite mensal de análises foi atingido.'},{status:429})
